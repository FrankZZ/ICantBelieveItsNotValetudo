...
class MapDrawer {
    ...
    draw(skipLayers) {
        ...
        const activeZone = this.mapData.entities.find(e => e.type === "active_zone");
        if (activeZone) {
            const activeZonePixels = this.translatePoints(pointsToCanvas(activeZone.points));
            ctx.fillStyle = "rgba(107, 244, 66, 0.3)";
            ctx.fillRect(activeZonePixels[0], activeZonePixels[1], activeZonePixels[2] - activeZonePixels[0], activeZonePixels[5] - activeZonePixels[1]);
        }
